...
/**
 * Returns the number of MICROSECONDS which have elapsed since the timer was
 * started. If the timer was never started, this function returns the number of
 * MICROSECONDS since the program started running. We compute the time elapsed
 * in microseconds and NOT in milliseconds because more precise calculations can
 * be made using microseconds.
 *
...
/**
 * Determines the progress percentage of time expired. A timer starts off at
 * zero percent progress. If, say, a timer needed to wait 10000 cycles and 7000
 * cycles have elapsed already since the timer was started, the percentage
 * returned is 0.7. For any timer which has already expired or which was never
 * started, the percentage returned is 1.0.
 *
 * @param timer_p:    The target timer used in determining the percent progress
 * elapsed
